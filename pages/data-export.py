...
def imgZipDownloadButton():
    if sst.createZip == 1:
        with st.status('Loading image data and compiling zip-archive, please wait ...', expanded = True) as zipStatus:        
            ...
            filename = sst.recordName + '_image-files.zip' if sst.userLoggedIn else 'epma-data-tools_test-dataset_image-files.zip'
...
def showKadiUpload():
        ...
        with uploadPlaceholder.container():      
            if not sst.csvMergedFiltered.empty and 'filterColumns' in sst.dataViewerFilter and sst.dataViewerFilter['filterColumns'] != []:
                if st.button('Upload current filter settings to Kadi4Mat', type='primary', key='kadiUp'):
                    kadiUploadFilters(uploadPlaceholder)
            else:
                st.info('No filters applied. Check out **' + fn.pageNames['viewer']['name'] + '** to filter you data first.', icon=fn.pageNames['viewer']['ico'])
...
if not sst.kadiLoaded:
    ...
    if not sst.csvMergedFiltered.empty and sst.dataViewerFilter['filterColumns'] != []:
        ...
        toggleDataFiltered = st.toggle('Include filtered data in file', 
                value=False, 
                disabled=True, 
                help='No filters applied. Check out **' + fn.pageNames['viewer']['name'] + '** to filter you data first.', 
                key='selectFilter')
    if not sst.csvMergedMinerals.empty:
        ...
        toggleDataMinerals = st.toggle('Include mineral predictions in file', 
                value=False, 
                disabled=True, 
                help='Please perform **' + fn.pageNames['mineral']['name'] + '** to include predicted minerals and calculated mineral formulas in the downloaded file.', 
                key='selectMineral')
    ...
    # images
    #########################   
    st.subheader('Download all image files (*.jpg, *.tif) as zip-archive', anchor=False)
    if len(sst.imageData) > 0:
        if sst.createZip == 1:
            imgZipDownloadButton()
        else:
            st.button('Click to compile images to zip-archive', type='primary', key='imgDown', on_click=compileZip)
    else:
    ...
    # kadi
    #########################
    st.subheader('Upload filter settings to Kadi4Mat', anchor=False)
